...
async def _fetch_price_history(
    ...
    # still serves the data -- with a maximum accepted span between 15 and
    # 21 days (15d -> 200, 21d -> 400, probed live). Retry across the
    # market's lifetime (createdAt .. endDate, padded a day each side) in
    # 14-day chunks at hourly fidelity -- the cadence the live snapshot
    # loop approximates.
    ...
    try:
        while cursor < final:
            result = await clob.get_price_history(
                yes_token,
                start_ts=cursor,
                end_ts=min(cursor + chunk, final),
                fidelity=60,
            )
            points.extend((p.t, p.p) for p in result.history)
            cursor += chunk
    except httpx.HTTPStatusError as e:
        status = e.response.status_code
        logger.warning(
    ...
    # Chunk boundaries can serve duplicate edge points; dedupe by timestamp.
    seen: set[int] = set()
    deduped = [(t, p) for t, p in points if not (t in seen or seen.add(t))]
    ...
    return (
        deduped,
        f"price_history: {len(deduped)} points via startTs/endTs range retry "
        "in 14-day chunks (interval query returned empty for this resolved "
        "market; ranges over ~15 days are rejected)",
    )
